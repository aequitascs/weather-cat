...
for (let y = 0; y < height; y += 1) {
  ...
  for (let x = 0; x < width; x += 1) {
    ...
    image.data[index] = Math.round(baseRed * (1 - fadeLevel));
    image.data[index + 1] = Math.round(baseGreen * (1 - fadeLevel));
    image.data[index + 2] = Math.round(blueLevel * 255);
